...
class Post(models.Model):
    ...
    def get_act_str(self):
        ...
        __url= self.user.get_profile().get_absolute_url()
        if get_language() == 'en':
            return u'<div class="span-13"><div class="span-2 last"><a href="%s"><img src="%s" width="50px" height="50px" /></a></div><div class="span-11 last">The user <a href="%s">%s</a> has answered your topic "%s". <a href="%s">View Answer</a></div></div>' % (__url,self.user.get_profile().foto.generate_url('50x50'.strip()),__url,self.user.username,self.topic.name,_url)
        else:
            return u'<div class="span-13"><div class="span-2 last"><a href="%s"><img src="%s" width="50px" height="50px" /></a></div><div class="span-11 last">El usuario <a href="%s">%s</a> respondió a tu tema "%s". <a href="%s">View Answer</a></div></div>' % (__url,self.user.get_profile().foto.generate_url('50x50'.strip()),__url,self.user.username,self.topic.name,_url)
