...
class AddQuickPostForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super(AddQuickPostForm, self).__init__(*args, **kwargs)
...
class ReputationForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        if commit:
            reputation.save()
            ###adding 1 point to user profile
            self.from_user.get_profile().add_points(1)
            #####
...
class MailToForm(forms.Form):
    subject = forms.CharField(label=_('Subject'),
                              widget=forms.TextInput(attrs={'size':'30', 'maxlength':'70', 'class':'longinput'}))
